...
// tool/runtime/pgb-tzdata.c and the two must agree: this one decides what gets
...
// ncurses honours TERMINFO; T-076 and experiments/97- carry the measurement.
func (b *Builder) buildTzdataData(rd, src string) error {
	if exists(filepath.Join(rd, "pgb-tzdata.o")) && exists(filepath.Join(rd, "pgb-tzdata-data.o")) {
		return nil
	}
	var roots []string
	...
	if v := os.Getenv("PGB_TZDATA_ZONES"); v != "" {
	...
	if err := b.compile(genPath, filepath.Join(rd, "pgb-tzdata-data.o"), "-O0"); err != nil {
		return err
	}
	return b.compile(filepath.Join(src, "pgb-tzdata.c"), filepath.Join(rd, "pgb-tzdata.o"), "-O2")
}
...
// runtime half checks them. ⚠ THE RUNTIME HALF IS tool/runtime/pgb-netdb.c
...
// The count is printed, and `pgb explain` repeats it.
func (b *Builder) buildNetdbData(rd, src string) error {
	if exists(filepath.Join(rd, "pgb-netdb.o")) && exists(filepath.Join(rd, "pgb-netdb-data.o")) {
		return nil
	}
	...
	if err := b.compile(genPath, filepath.Join(rd, "pgb-netdb-data.o"), "-O0"); err != nil {
		return err
	}
	return b.compile(filepath.Join(src, "pgb-netdb.c"), filepath.Join(rd, "pgb-netdb.o"), "-O2")
}
