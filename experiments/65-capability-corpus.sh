...
#
#   id | category | nixpkgs attribute | program | mode | assertion | extras
#
...
gl-1|OpenGL / EGL|mesa-demos|glxgears|gui|GL_RENDERER|mesa
gl-2|OpenGL / EGL|glmark2|glmark2|gui|GL_RENDERER|mesa
gl-3|OpenGL / EGL|mpv|mpv|cli|mpv [0-9]|mesa
vulkan-1|Vulkan|vulkan-tools|vulkaninfo|cli|(lavapipe|llvmpipe|Vulkan Instance)|mesa
...
windows_named() { DISPLAY="$XDISP" xwininfo -root -tree 2>/dev/null | grep -ci "$1" || true; }
...
for line in $CORPUS; do
  ...
  for name in $ENVS; do
    ...
    _q=0
    while [ "$_q" -lt 10 ] && [ "$(windows_named "$prog")" != 0 ]; do sleep 1; _q=$((_q+1)); done
