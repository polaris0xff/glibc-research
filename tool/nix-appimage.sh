...
# ⛔ THE HARDLINKS GO AT THE TOP LEVEL, NOT UNDER bin/, AND THAT IS NOT A
# STYLE CHOICE. sharun takes its AppDir from its OWN location: a hardlink at
# `$APPDIR/bin/<name>` makes it treat `$APPDIR/bin` as the root, where there is
# no `.env` and no `lib`. Measured on kdenlive: the first version of this
# selector exec'd `$APPDIR/bin/kdenlive`, `.env` was NEVER OPENED (checked in
# an strace: zero opens of it), QT_PLUGIN_PATH therefore never reached the
# process, Qt searched /usr/lib/qt-6/plugins and the mount's own bin/, found no
# platform plugin and called abort() -- **with no output at all**, which is
# the worst shape a bundling failure can take.
for _p in "$APPDIR"/shared/bin/*; do
  [ -f "$_p" ] || continue
  _n=$(basename "$_p")
  # ⚠ A program named like part of the layout would overwrite it.
  case "$_n" in AppRun|sharun|bin|lib|lib32|share|shared|store|.env) continue ;; esac
  ln -f "$APPDIR/sharun" "$APPDIR/$_n" 2>/dev/null || cp "$APPDIR/sharun" "$APPDIR/$_n"
  # bin/ keeps a copy too: it is what `.env`'s PATH points at, so a program
  # that shells out to a sibling finds it.
...
# (`useful-tools/AppRun.sh` at da7649b9): if argv0 names a program run that;
# else if the FIRST ARGUMENT names one, run that and drop it; else run the main
# program. ⛔ Without this an artefact carrying six programs could only ever
# start one of them, because sharun dispatches on argv0 and an AppImage is
# always invoked by the image's own name.
# ⛔ AND THE SELECTOR COSTS THE HOST'S /bin/sh, WHICH IS MEASURED RATHER THAN
# ASSUMED. A shell AppRun is a script, so the kernel runs the HOST's
# interpreter, and that interpreter loads the HOST's libc. `experiments/90-`
# saw exactly that: our kdenlive bundle opened 1-4 host shared objects on every
# glibc row and none on the four musl ones -- and
# `pkgforge-dev/kdenlive-AppImage-Enhanced`, whose AppRun is also a shell
# script, showed the same shape (up to 10 on Rocky 8). ⭐ The shebang cannot
# point into the bundle: the mount path is not known until run time.
#
# ⭐ So a single-program bundle does not pay it. AppRun is a hardlink of sharun
# there, exactly as before this selector existed, and 85-/86-/89-'s zero-host-
# object rows are unaffected. Only a bundle carrying several programs takes the
# shell, and the run says so.
if [ "$(ls "$APPDIR"/shared/bin 2>/dev/null | wc -l)" -le 1 ]; then
  ln -f "$APPDIR/sharun" "$APPDIR/AppRun" 2>/dev/null || cp "$APPDIR/sharun" "$APPDIR/AppRun"
  say "apprun      sharun itself (one program: no shell, no host interpreter)"
elif cc -O2 -static -DPGB_APPRUN_DEFAULT='"'"$PROG"'"' \
       -o "$APPDIR/AppRun" "$SELF/runtime/pgb-apprun.c" >/dev/null 2>&1 \
     && [ "$(readelf -l "$APPDIR/AppRun" 2>/dev/null | grep -c INTERP)" = 0 ]; then
  # ⭐ A STATIC SELECTOR, NOT A SHELL SCRIPT. tool/runtime/pgb-apprun.c carries
  # the measurement: a script is run by the HOST's interpreter, which loads the
  # HOST's libc, and `experiments/90-` saw 1-4 host shared objects on every
  # glibc row because of it. `QaidVoid/onelf` dispatches its entrypoints from a
  # static musl stub and pays nothing; this is the same idea built by the
  # toolchain this repository exists to build.
  # ⚠ The `readelf` check is not decoration: a `cc` that quietly produced a
  # DYNAMIC binary would put the host's loader back in the bundle, which is the
  # exact thing being removed.
  say "apprun      pgb-apprun, static, $(wc -c < "$APPDIR/AppRun") bytes (no host interpreter)"
else
  warn "⚠ no static AppRun could be built, so it falls back to a SHELL SCRIPT"
  warn "   selector -- which loads the HOST's /bin/sh and its libc."
...
export APPDIR
# ⛔ The exec target is at the TOP LEVEL so sharun's AppDir is \$APPDIR itself.
