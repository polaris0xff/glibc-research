...
if [ "$(ls "$APPDIR"/shared/bin 2>/dev/null | wc -l)" -le 1 ]; then
  ...
  warn "⚠ this bundle carries several programs, so AppRun is a SHELL SCRIPT"
  warn "   selector -- which loads the HOST's /bin/sh and its libc. Build with"
  warn "   one program if criterion 2 (no host shared object) must hold."
