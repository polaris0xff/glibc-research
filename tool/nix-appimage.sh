...
TARGET=""; OUT=""; NAME=""; SELFTEST=0; KEEP=0
while [ $# -gt 0 ]; do
  case "$1" in
    --out)      shift; OUT="${1:-}" ;;
    --name)     shift; NAME="${1:-}" ;;
...
for d in "$ROOT"/*/share; do
  [ -d "$d" ] || continue
  cp -aL "$d/." "$APPDIR/share/" 2>/dev/null || true
done
...
# XDG_DATA_DIRS so the bundled share/ is found. ⚠ Appended, never replacing:
# an application that also wants the host's icon theme should still get it.
cat > "$APPDIR/.env" <<'EOF'
XDG_DATA_DIRS=${SHARUN_DIR}/share:${XDG_DATA_DIRS}:/usr/local/share:/usr/share
GCONV_PATH=${SHARUN_DIR}/lib/gconv
GTK_PATH=${SHARUN_DIR}/lib/gtk-3.0
GDK_PIXBUF_MODULE_FILE=${SHARUN_DIR}/lib/gdk-pixbuf-2.0/2.10.0/loaders.cache
FONTCONFIG_PATH=${SHARUN_DIR}/share/fontconfig/conf.d
EOF
