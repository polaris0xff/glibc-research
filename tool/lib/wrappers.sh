...
build_runtime() {
  ...
  if [ "$EMBED_TERMINFO" = 1 ]; then
    build_terminfo_data "$rd" "$CCB"
  fi
...
# Turn a handful of compiled terminfo entries into a C file of bytes.
#
# ⛔ A HANDFUL, NOT A DATABASE, and the restraint is the point.
# docs/limitations.md §3 says the argument for a glibc portability tool owning
# a TERMINAL database is weak, and it is. What a program actually needs is a
# description of the terminal it is talking to, and on a machine with no
# terminfo at all that is nearly always one of these. ncurses still consults
# its compiled-in path after $TERMINFO, so a host entry this binary does not
# carry is still reachable.
#
# ⚠ Entries come from the pinned environment, which has ncurses' own compiled
# ones, so two machines building the same source embed the same descriptions.
PGB_TERMINFO_ENTRIES="${PGB_TERMINFO_ENTRIES:-xterm xterm-256color xterm-color \
screen screen-256color tmux tmux-256color linux vt100 vt220 ansi rxvt putty dumb}"

build_terminfo_data() {
  rd="$1"; CCB="$2"
  [ -f "$rd/pgb-terminfo.o" ] && [ -f "$rd/pgb-terminfo-data.o" ] && return 0
  # ⛔ SEARCH EVERY ROOT FOR EVERY ENTRY, and do not pick "the first root that
  # exists". Debian 12 HAS /usr/share/terminfo -- as an empty shell -- and
  # keeps the actual entries in /lib/terminfo, so choosing the first existing
  # DIRECTORY found a database with nothing in it and the generator reported
  # that none of fourteen common terminals existed. Same shape as the libm
  # linker script in experiments/73-: the first candidate that exists is not
  # necessarily the one with the content.
  roots=""
  for c in /usr/share/terminfo /lib/terminfo /etc/terminfo /usr/lib/terminfo; do
    [ -d "$c" ] && roots="$roots $c"
  done
  [ -n "$roots" ] || die "--embed-terminfo needs a terminfo database in the build environment (install ncurses-base)" 2
  gen="$rd/pgb-terminfo-data.c"
  found=""
  {
    printf '/* generated by pgb from%s -- do not edit */\n' "$roots"
    printf 'struct pgb_ti_file { const char *name; const unsigned char *data; unsigned len; };\n'
    i=0
    for t in $PGB_TERMINFO_ENTRIES; do
      f=""
      for _root in $roots; do
        cand="$_root/$(printf '%s' "$t" | cut -c1)/$t"
        [ -f "$cand" ] && { f="$cand"; break; }
      done
      [ -n "$f" ] || continue
      printf 'static const unsigned char t%d[] = {' "$i"
      od -An -v -tu1 < "$f" | tr -s ' ' | tr ' ' '\n' | grep -v '^$' | tr '\n' ','
      printf '};\n'
      found="$found $t:$i"
      i=$((i+1))
    done
    printf 'const struct pgb_ti_file pgb_ti_files[] = {\n'
    for e in $found; do
      printf '  { "%s", t%s, sizeof t%s },\n' "${e%%:*}" "${e##*:}" "${e##*:}"
    done
    printf '};\n'
    printf 'const unsigned pgb_ti_nfiles = %d;\n' "$i"
  } > "$gen"
  [ -n "$found" ] || die "--embed-terminfo found none of: $PGB_TERMINFO_ENTRIES under $root" 2
  vsay "embedding terminfo entries:$found"
  $CCB -O0 -fno-lto -c -o "$rd/pgb-terminfo-data.o" "$gen" || die "terminfo data build failed"
  $CCB -O2 -fno-lto -c -o "$rd/pgb-terminfo.o" "$PGB_SELF/tool/runtime/pgb-terminfo.c" \
    || die "terminfo shim build failed"
}
...
link_flags() {
  ...
  if [ "$EMBED_TERMINFO" = 1 ]; then
    printf -- ' -Wl,-u,pgb_terminfo_anchor %s %s' "$rd/pgb-terminfo.o" "$rd/pgb-terminfo-data.o"
  fi
