...
| **glibc floor** | there is a floor at **2.34**, because `files`/`dns` became builtin there. ✅ **Measured**, not reasoned: `experiments/21-glibc-version-floor.sh` builds the same source at 2.31 and at 2.36 against the same target, and below the floor the override **moves** the `dlopen` rather than removing it — `libnss_files.so.2` and `libnss_dns.so.2` are opened with and without it. `docs/history/corrections.md` C6 has the table. ⭐ **The build image is pinned at 2.41, not at the floor**: nothing pushes the pin down (the output is static), and the `--host-dlopen` **ceiling** pushes it up. T-070 moved it 2.36 → 2.41 with all four costs measured at zero, and the 2.41 arm of 21- still reports `none`. `docs/design/glibc-versions.md`. ⚠ This row said "reasoned, not measured — planned and unwritten" for most of the project's life and was left stale after the experiment landed. |
