...
| all **54** experiments | ✅ **every one measured, except the two named here as pre-registered and NOT YET RUN.** ⭐ The five newest were pre-registered and committed before they ran: `104-` (does our compiled-in loader leak symbols between objects? — it did, **C46**), `105-` (`file`(1) out of a bundle with no `MAGIC` variable and no per-application `AppRun` — **11/11**, control 0/11), `106-` (the two criteria this tree had parked as *unmeasurable in this bed* — a non-C **locale** and a host **theme** — measured against a **fixture** the tree now builds: 7/7 and 11/11, both with controls) ⭐ `107-` (the corpus's last unexplained row — **run**: 9 pass, 1 fail, and the fail is the finding, **C55**) and ⭐ `108-` (⛔ flameshot's capture, which the record has called *Untried* since it was written). ⚠ `108-` is committed and **has not run yet**. ⚠ The count is `ls experiments/[0-9]*-*.sh \| wc -l` and nothing else — `clock.sh` and `lib.sh` are **libraries**. `86-` writes one evidence file per subject (`RESULT.jq.txt`, `RESULT.mpv.txt`) because it runs against two |
...
| ⭐ **the capability corpus, three subjects per category** | ✅ **T-080 COMPLETE — 26 of 26 subjects**, every row a count out of eleven, 0 UNRESOLVED and 0 INSTRUMENT. ⭐ **SIX CATEGORIES CLOSED at three subjects each**: **GTK 3** (galculator, mousepad, geany), **X11/XCB** (xeyes, xclock, xterm), **OpenGL/EGL** (eglinfo, glxgears, glmark2), **Qt** (qalculate-qt, keepassxc, qbittorrent), **SDL** (dosbox, stella, scummvm) and **Vulkan** (the third, `vkmark`, is a **bed** limit — it needs `/dev/dri`, which exists nowhere here). ⛔ **"passing" and "clean" are NOT the same count and this row used to merge them**: `qalculate-qt` passes 11/11 and is clean on **4/11**, which `experiments/107-` has now explained — it spawns `gnuplot` through the host's `/bin/sh` (**C55**, **T-094**). ⚠ And the clean counts predate two instrument fixes (**C49**, **C54**) that owe one corpus re-run. ⛔ **Read a corpus zero as a question, never a result**: of the first four, two were the instrument (C34, C36), one a bundler bug now fixed (C37), one the closure's own glibc (C35) |
