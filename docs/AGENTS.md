...
| all **48** experiments | ✅ every one measured. ⚠ The count is `ls experiments/[0-9]*-*.sh \| wc -l` and nothing else — `clock.sh` and `lib.sh` are **libraries**. `86-` writes one evidence file per subject (`RESULT.jq.txt`, `RESULT.mpv.txt`) because it runs against two |
...
| bundle vs. the field | ⭐ **LEVEL OR AHEAD ON SPEED, ahead on cleanliness and one-command packaging, behind on size.** `jq` cold **58.3 vs 58.4 ms** (parity); **kdenlive cold 380 vs 514 ms — 0.74×, ours faster**, with **payload clean 11 of 11 for ours against 4 of 11 for theirs** ⛔ (**BOTH of those numbers are the ones `TODO/ci.md` T-084 puts at risk, ours included.** `experiments/90-` carries a hand copy of the trace classifier, it calls **`tree`** mode, and its test script invokes the artefact **twice** — so C38's unconditional clear **fires** and both counts describe only the **second** invocation. ⚠ Measured by `experiments/102-` arm S: of the six experiments carrying a copy, `90-` is the **only** one C38 reaches. How much it moves is not measured). ⛔ Both read against us that morning; two constants in `internal/bundle/appimage.go` closed it. ⚠ kdenlive's **warm** row is 3.45× against us and unexplained, and its render direction is unresolved. `TODO/poc.md` T-055, `TODO/research.md` T-057 |
...
| ⭐ **the capability corpus, three subjects per category** | ⏳ **T-080, running.** ⭐ **FOUR CATEGORIES CLOSED, all three subjects passing in each**: **GTK 3** (galculator, mousepad, geany), **X11/XCB** (xeyes, xclock, xterm), **OpenGL/EGL** (eglinfo, glxgears, glmark2), **Qt** (qalculate-qt, keepassxc, qbittorrent). **Vulkan** is 3 of 3 with the third a **bed** limit — `vkmark` needs `/dev/dri`, which exists nowhere here. ⭐ **SDL is open at 1 of 3 and the first row works**: `dosbox` **11/11**, clean **11/11**, 181 store paths compiled in. ⛔ **Read a corpus zero as a question, never a result**: of the first four, two were the instrument (C34, C36), one a bundler bug now fixed (C37), one the closure's own glibc (C35) |
