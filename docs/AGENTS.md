...
| bundle vs. the field | ⭐ **LEVEL OR AHEAD ON SPEED, ahead on cleanliness and one-command packaging, behind on size.** `jq` cold **58.3 vs 58.4 ms** (parity); **kdenlive cold 380 vs 514 ms — 0.74×, ours faster**, with host objects **0 of 11 against 4 of 11** ⚠ (**that 4 is the number `TODO/ci.md` T-084 puts at risk**: `experiments/90-` still carries a hand copy of the trace classifier that counted FAILED opens as loads. Our 0 stands — the defect only runs one way). ⛔ Both read against us that morning; two constants in `internal/bundle/appimage.go` closed it. ⚠ kdenlive's **warm** row is 3.45× against us and unexplained, and its render direction is unresolved. `TODO/poc.md` T-055, `TODO/research.md` T-057 |
...
| ⭐ **the capability corpus, three subjects per category** | ⏳ **T-080, running.** ⭐ Closed so far: **GTK 3** (galculator, mousepad, geany), **X11/XCB** (xeyes, xclock — `xterm` is C37), **OpenGL/EGL** (`eglinfo`, `glxgears`), each **11/11 pass and 11/11 clean**; **Vulkan** 1 of 3 at 11/11. ⛔ **Read a corpus zero as a question, never a result**: of the first four, two were the instrument (C34, C36), one a bundler bug now fixed (C37), one the closure's own glibc (C35) |
