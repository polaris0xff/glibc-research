...
| all **53** experiments | ✅ **every one measured.** ⭐ The four newest were pre-registered and committed before they ran: `104-` (does our compiled-in loader leak symbols between objects? — it did, **C46**), `105-` (`file`(1) out of a bundle with no `MAGIC` variable and no per-application `AppRun` — **11/11**, control 0/11), `106-` (the two criteria this tree had parked as *unmeasurable in this bed* — a non-C **locale** and a host **theme** — measured against a **fixture** the tree now builds: 7/7 and 11/11, both with controls) and ⭐ `107-` (the corpus's one unexplained row: `qalculate-qt` passes 11/11 and is clean on only 4/11). ⚠ The count is `ls experiments/[0-9]*-*.sh \| wc -l` and nothing else — `clock.sh` and `lib.sh` are **libraries**. `86-` writes one evidence file per subject (`RESULT.jq.txt`, `RESULT.mpv.txt`) because it runs against two |
