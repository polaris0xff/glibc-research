...
| all **34** experiments | ✅ every one measured. ⚠ **The count read 24 until 2026-09-03c**, when `ls experiments/[0-9]*-*.sh \| wc -l` was actually run against it. 33 write `evidence/<NN>-*/RESULT.txt`; `86-` writes one per subject (`RESULT.jq.txt`, `RESULT.mpv.txt`) because it runs against two. ⭐ `experiments/clock.sh` is a **library**, not an experiment — the wall-clock instrument, beside `lib.sh` |
