...
| ⭐ **the capability corpus, three subjects per category** | ⏳ **T-080, running.** ⭐ **THREE CATEGORIES CLOSED, all three subjects passing in each**: **GTK 3** (galculator, mousepad, geany), **X11/XCB** (xeyes, xclock, xterm), **OpenGL/EGL** (eglinfo, glxgears, glmark2). **Vulkan** is 3 of 3 with the third a **bed** limit — `vkmark` needs `/dev/dri`, which exists nowhere here. ⛔ **Read a corpus zero as a question, never a result**: of the first four, two were the instrument (C34, C36), one a bundler bug now fixed (C37), one the closure's own glibc (C35) |
