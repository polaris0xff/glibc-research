...
| [`design/nix-front-end.md`](design/nix-front-end.md) | ⭐ **the operator's ruling that nixpkgs IS the planner**, the two recipe shapes at a pinned soarpkgs commit, and the "use nix without installing nix" reading list. ⛔ Nothing on it is built or verified; the mining is the next session's first task |
