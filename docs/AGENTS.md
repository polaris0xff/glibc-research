...
| all **52** experiments | ✅ **every one measured.** ⭐ The three newest were pre-registered and committed before they ran: `104-` (does our compiled-in loader leak symbols between objects? — it did, **C46**), `105-` (`file`(1) out of a bundle with no `MAGIC` variable and no per-application `AppRun` — **11/11**, control 0/11) and ⭐ `106-` (the two criteria this tree had parked as *unmeasurable in this bed* — a non-C **locale** and a host **theme** — measured against a **fixture** the tree now builds: 7/7 and 11/11, both with controls). ⚠ The count is `ls experiments/[0-9]*-*.sh \| wc -l` and nothing else — `clock.sh` and `lib.sh` are **libraries**. `86-` writes one evidence file per subject (`RESULT.jq.txt`, `RESULT.mpv.txt`) because it runs against two |
