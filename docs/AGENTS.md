...
| all **49** experiments | ⏳ **48 measured, one written and not yet run** — `103-`, T-091's decode row, pre-registered and committed before it runs. ⚠ The count is `ls experiments/[0-9]*-*.sh \| wc -l` and nothing else — `clock.sh` and `lib.sh` are **libraries**. `86-` writes one evidence file per subject (`RESULT.jq.txt`, `RESULT.mpv.txt`) because it runs against two |
