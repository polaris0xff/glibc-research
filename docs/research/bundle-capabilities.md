...
| **SDL** | ⛔ **STILL NOT MEASURED — no row recorded.** Three subjects are queued in `65-` (`dosbox`, `stella`, `scummvm`) and none has finished. Until one does this stays a hypothesis, graded *Excellent* by the field | §1; `65-` `sdl-1..3` |
| ⭐ **Qt** | ⏳ **2 of 3 in, both passing.** `qalculate-qt` **11/11** (clean **4/11**, unexplained and recorded as such), `keepassxc` **11/11** clean **11/11**. `qbittorrent` running | `65-` `qt-1..3` |
