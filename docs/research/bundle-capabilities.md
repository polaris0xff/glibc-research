...
| ⭐ `qalculate-qt` | ⛔ **the corpus's last unexplained row, now MEASURED** (`experiments/107-`, 9 pass 1 fail). It passes 11/11 and is clean on exactly the four **musl** rows. ⭐ The seven glibc rows load **1 to 4** host objects and every one belongs to the **shell the environment ships** — `dash` costs `libc.so.6`, `bash` costs `libc.so.6 libdl.so.2 libtinfo.so.6` — because `qalculate-qt` probes for **gnuplot** through `/bin/sh -c --`. On musl that exec dies `exited with 127` having loaded nothing. ⭐ **Zero Qt/X11/GL host objects on every row: the bundler is not leaking.** ⛔ Two of my own pre-registered claims fell here and both are kept (**C55**): the *static busybox* story (refuted by `readelf` before the run) and the *DBus autolaunch* story (refuted by arm B, which carries `dbus-launch` and is clean on the same four rows). ⚠ **T-094** owns what remains: an application that shells out to the host loads the host's libc through that shell, and no path rewriting prevents it |
| ⭐ `flameshot` | ⛔ **the SUBJECT'S SHAPE, and it is now two separate things.** It is a tray/daemon screenshot tool: `flameshot` with no subcommand draws nothing by design, and the only window it puts on the server is a **3×3 `Qt Selection Owner`** — so a `gui` row demanding a toplevel ≥50×50 cannot pass on it, however well the bundle works. ⭐ The **session-bus** half is **FIXED**: the bundle carries `dbus-daemon` (`--with-program`) and the bed carries the config file (`bed-fixtures.sh --install dbus`), and the `Unable to connect via DBus` error is gone. ⛔ **What remains is the capture**: `flameshot full --path …` answers `Unable to capture screen` under `Xvfb`, with `QT_QPA_PLATFORM=xcb` forced. ⚠ Its closure carries `grim`, which is **Wayland-only**, and no `xdg-desktop-portal`. ⭐ **The route is named rather than the limit declared**: pull `xdg-desktop-portal` and a backend in with `--extra` and run it beside the bus. ⭐ **[`experiments/108-`](../../experiments/108-flameshot-capture.sh) now does exactly that** — pre-registered, with the capture judged on a FILE (a PNG whose IHDR width and height match the server's) rather than on a log line, and with an instrument control that dumps the same display with `xwd` from OUTSIDE first, so "no screenshot" cannot be read as a bundler result when the bed simply cannot be grabbed |
...
| ⛔ *"you are no longer able to use vulkan layers like mangohud"* | ⛔ **STILL NOT MEASURED, and it is now the ONLY row here without a measurement.** A Vulkan layer is a host `.so` loaded at run time, which is exactly what `--host-dlopen` is for — but nobody has pointed it at a layer. ⭐ **A real one is in reach without a fixture**: the mesa closure every GL bundle here carries ships `VkLayer_MESA_overlay` — MangoHud's own class — so the claim can be tested against a genuine overlay layer. ⚠ The discriminator is the **path** the layer is opened from, since a bundle carrying its own `libvulkan.so.1` decides the search itself. **[TODO/research.md T-093](../../TODO/research.md)** owns it |
...
| ⭐ *"it seems none of the solutions implement `dlmopen`, so you are likely to run into a lot of symbol collisions"* | ⛔ **IT LANDED ON US, AND IT IS NOW FIXED.** ⚠ This row used to answer from the DESIGN — *"nothing is added to a global search scope"* — and said so. `experiments/104-` measured it and the design reading was **wrong**: `pgb_elf_dlopen` opened with `(void)flags;` and `el_resolve` searched **every** loaded object before the requester's own, so the first object loaded won every name. ⭐ **Fixed to ld.so's order** and re-measured: ours now gives the **same answer as glibc's `ld.so` in both scopes**, and the two answers differ from each other, so the agreement is not a constant. **C46** |
...
| *"we also have to set `GCONV_PATH` and good luck figuring out which gconv plugin your app needs … when the plugin is missing there is no error"* | ⭐ **The static path does not have this problem at all**: `--wrap=iconv_open` onto static GNU libiconv, so no gconv module is ever looked for. ⭐ **AND IT IS NOW MEASURED** (`experiments/30-`, `pass=24 fail=0 skip=0`) — the arm had been silently SKIPPED on every previous run (**C56**): a plain `-static` probe opens **1 of 12** encodings on musl and **crashes outright** on all seven glibc rows; with the wrap it opens **12 of 12 with a byte-exact round trip on all eleven**, for ~1.9 MiB |
