...
| ⭐ **SDL** | ⏳ **1 of 3 in, and it works.** `dosbox` draws a real toplevel on **11 / 11** with **zero host shared objects on 11 / 11** — **181** store paths compiled in, 169 resolving, the largest closure measured here. ⛔ It was a *hypothesis* graded *Excellent* by the field until this row; `stella` and `scummvm` are still queued | §1; `65-` `sdl-1..3` |
