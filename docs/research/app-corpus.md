...
| 3 · GTK's locale prefix | ⭐ **THE BED NOW HAS A NON-C LOCALE — 2026-09-04c.** `scripts/common/bed-fixtures.sh` compiles `de_DE.UTF-8` and installs it, and `experiments/106-` measures it: `setlocale` succeeds on **7 of 7 glibc rows**, the `xx_XX.UTF-8` control fails on **7 of 7**, and the locale is genuinely in effect (`CODESET=UTF-8`, decimal `,`). ⛔ The 4 musl rows are `n/a` by musl's design, not a gap. ⚠ What is still owed is the **catalogue** itself — an application that ships a `.mo` — which is this rung's own row |
