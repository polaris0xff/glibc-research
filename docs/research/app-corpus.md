...
| ⛔ **a HOST GTK/icon THEME** | ⭐ **Measured 2026-09-04c: `0 of 11` rootfs carry anything under `usr/share/themes` or `usr/share/icons`** — they are minimal container images. So "does the bundle follow the system theme" cannot fire here for the same reason rung 3's locale criterion cannot: there is nothing on the host to follow. ⚠ **The measurable half is different and is worth keeping separate**: whether the bundle *asks* the host. Its `.env` puts `/usr/local/share` and `/usr/share` in `XDG_DATA_DIRS` after its own, so the search order is right; ⭐ the build host **does** have themes (`Adwaita`, `Humanity`), so the ask is measurable there, on one environment |
