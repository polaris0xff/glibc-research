...
### ⛔ The rows that came out against us — two closed, one still open

⭐ **T-078's rule**: *"If a row comes out against us, that row IS the
deliverable — report it, do not soften the axis until it passes."*

1. ✅ **The environment-default codeset** — the one axis native musl beat both
   glibc columns. With no `LANG` set, musl answers UTF-8 on 11 of 11 and every
   glibc arm answered `ANSI_X3.4-1968` on 11 of 11. `pgb build --utf8-default`
   now answers **UTF-8 on 11 of 11** (`experiments/67-`, `pass=7 fail=0
   skip=0`, two runs identical). ⛔ **The row that makes that mean something is
   `LANG=C`**: a mechanism forcing UTF-8 unconditionally would score the
   headline row green and be a *bug*. U5 measures exactly that and is green on
   11 of 11. ⚠ `--embed-locale` alone does **not** move the axis (U3), and
   `C.UTF-8` is the C locale with a UTF-8 charset — this says nothing about
   collation or message catalogues.
2. ✅ **`/etc/services` and `/etc/protocols`** — the eleventh host-data
   dependency. All three columns failed on the same three environments, every
   one a glibc row shipping no `/etc/services`. `--embed-netdb` now answers
   `http/tcp = 80` and `tcp = 6` on **11 of 11** (`experiments/66-`, `pass=12
   fail=0 skip=0`, two runs identical). ⭐ **The host still wins where it has
   the file** — the wrappers call `__real_*` first — and a name *nobody*
   carries still answers NULL on 11 of 11, so it is a fallback and not an
   override. ⚠ Its tables cost **−7,280 B**: the arm with them is smaller.
3. ⛔ **STILL AGAINST US: `getaddrinfo` with a service name.**
   `getaddrinfo(NULL, "http", …)` resolves on **8 of 11** with `--embed-netdb`
   on. `--wrap` redirects the *public* symbol; glibc's `getaddrinfo` reaches
   its own internal `__getservbyname_r`, which the linker cannot interpose.
   ⭐ **Pre-registered as a failure before the run** (`experiments/66-` N4), so
   a green result would have had to be explained rather than pocketed. A
   program that needs it must pass the port number.

### What the matrix says about the parity claim

⭐ **On every axis measured here, `pgb` is level with or ahead of native musl
static** — ahead on iconv (12/12 vs 10/12), timezone (11 vs 7),
`/etc/services` (11 vs 8) and throughput (84× on contended malloc), level on
the environment-default codeset once `--utf8-default` is passed, at 11.5× the
artefact size. ⛔ **The claim *"no buts and no ifs"* is still not true**: the
two axes that were flatly lost are now **flags a developer must pass**, not
defaults, and `getaddrinfo` with a service name is a measured "but" no flag
closes. ⚠ A capability behind an opt-in flag is a capability the tool *has*; it
is not parity with a libc that does it unconditionally.
...
| onelf in its preferred modes | ⛔ **not measured.** memfd, FUSE and tmpfs all need `unshare(CLONE_NEWUSER\|CLONE_NEWNS)`, which is EPERM inside this chroot bed, so every onelf row is its last fallback — cache mode. ⭐ `experiments/69-` isolates the cause to `chroot` itself (the host permits all five namespaces; `unshare --mount` alone permits it; `pivot_root` into the same rootfs permits it), so this is a property of how the bed is ENTERED rather than of the machine. Its coverage result is unaffected: the failures are gconv, not delivery |
